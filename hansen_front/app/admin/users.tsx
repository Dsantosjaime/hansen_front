...
export default function UsersScreen() {
  ...
  const { data: userRulesGroups = [] } = useGetUserRulesGroupsQuery();

  const [deleteUser] = useDeleteUserMutation();
  const [modifyUser] = useModifyUserMutation();
  const [addUser] = useCreateUserMutation(); // RTK Query: createUser (alias "addUser")
  ...
  const roleOptions = useMemo<SelectOption<string>[]>(
    () =>
      userRulesGroups.map((g: UserRulesGroup) => ({
        value: g.id,
        label: g.name,
      })),
    [userRulesGroups]
  ...
  }, [draftEmail, draftName, draftRoleId]);
  ...
  const saveCreate = useCallback(async () => {
    ...
    setDraftName("");
    setDraftRoleId(null);
  }, [addUser, canSave, draftEmail, draftName, draftRoleId]);
