...
export default function PluginParamsScreen() {
  ...
  const omitedFunctions = useMemo(
    () =>
      (pluginParams ?? [])
        .filter((p) => p.type === PluginParamType.FUNCTION)
        .map((p) => ({ id: p.id, function: p.name })),
    [pluginParams]
  );
  ...
  const nameDiscriminators = useMemo(
    () =>
      (pluginParams ?? [])
        .filter((p) => p.type === PluginParamType.NAME)
        .map((p) => ({ id: p.id, name: p.name })),
    [pluginParams]
  );

  // --------------------
  // Mutations (CRUD server)
  // --------------------
  const [createPluginParam] = useCreatePluginParamMutation();
  const [updatePluginParam] = useUpdatePluginParamMutation();
  const [deletePluginParam] = useDeletePluginParamMutation();
  ...
  const onAddOmited = useCallback(async () => {
    ...
    await createPluginParam({
      name: v,
      type: PluginParamType.FUNCTION,
    }).unwrap();

    setNewOmited("");
  }, [createPluginParam, newOmited]);
  ...
  const onAddName = useCallback(async () => {
    ...
    await createPluginParam({
      name: v,
      type: PluginParamType.NAME,
    }).unwrap();

    setNewName("");
  }, [createPluginParam, newName]);
  ...
  const onCommitOmited = useCallback(
    async (id: string) => {
      ...
      await updatePluginParam({
        id,
        data: { name: v, type: PluginParamType.FUNCTION },
      }).unwrap();

      setOmitedDrafts((prev) => ({ ...prev, [id]: v }));
    },
    [updatePluginParam, omitedDrafts]
  );
  ...
  const onCommitName = useCallback(
    async (id: string) => {
      ...
      await updatePluginParam({
        id,
        data: { name: v, type: PluginParamType.NAME },
      }).unwrap();

      setNameDrafts((prev) => ({ ...prev, [id]: v }));
    },
    [updatePluginParam, nameDrafts]
  );

  const onDeleteOmited = useCallback(
    async (id: string) => {
      await deletePluginParam({ id }).unwrap();
      setOmitedDrafts((prev) => {
        ...
        {isError ? (
          <View style={styles.centerPad}>
            ...
            <Text style={{ color: muted, marginTop: 6 }}>
              {String((error as any)?.data ?? (error as any)?.error ?? "")}
            </Text>
          </View>
        ) : (
          <View style={styles.contentRow}>
            {/* LEFT: Omited Functions */}
